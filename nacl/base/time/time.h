...
// Add or subtract |value| from a TimeDelta. The int64_t argument and return value
...
class TimeBase {
 public:
  ...
  static const int64_t kMillisecondsPerDay = kMillisecondsPerSecond * 60 * 60 *
                                           kHoursPerDay;
...
inline TimeDelta TimeDelta::FromSecondsD(double secs) {
  ...
  if (secs == std::numeric_limits<double>::infinity())
    return Max();
  return TimeDelta(static_cast<int64_t>(secs * Time::kMicrosecondsPerSecond));
}
...
inline TimeDelta TimeDelta::FromMillisecondsD(double ms) {
  ...
  if (ms == std::numeric_limits<double>::infinity())
    return Max();
  return TimeDelta(static_cast<int64_t>(ms * Time::kMicrosecondsPerMillisecond));
}
