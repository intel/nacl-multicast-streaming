...
// IMPORTANT: This is thread safe in the original implementation, but since we
// don't need this on the NaCl implementation, just remove the thread safety
// and the required included files with it.
...
base::TimeTicks ClockNow(clockid_t clk_id) {
  ...
  absolute_micro =
      (static_cast<int64_t>(ts.tv_sec) * base::Time::kMicrosecondsPerSecond) +
      (static_cast<int64_t>(ts.tv_nsec / base::Time::kNanosecondsPerMicrosecond));
...
static const int64_t kWindowsEpochDeltaSeconds = UINT64_C(11644473600);
...
Time Time::FromTimeVal(struct timeval t) {
  PP_DCHECK(t.tv_usec < static_cast<int>(Time::kMicrosecondsPerSecond));
  PP_DCHECK(t.tv_usec > 0);
