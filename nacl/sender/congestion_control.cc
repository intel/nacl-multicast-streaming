...
class AdaptiveCongestionControl : public CongestionControl {
 public:
  AdaptiveCongestionControl(base::TickClock* clock,
                            uint32_t max_bitrate_configured,
                            uint32_t min_bitrate_configured,
  ...
  void SendFrameToTransport(uint32_t frame_id, size_t frame_size,
  ...
  uint32_t GetBitrate(base::TimeTicks playout_time,
 ...
 private:
  ...
  base::TimeTicks EstimatedAckTime(uint32_t frame_id, double bitrate);
  ...
  base::TimeTicks EstimatedSendingTime(uint32_t frame_id, double bitrate);

  base::TickClock* const clock_;  // Not owned by this class.
  const uint32_t max_bitrate_configured_;
  const uint32_t min_bitrate_configured_;
  const double max_frame_rate_;
  std::deque<FrameStats> frame_stats_;
  uint32_t last_frame_stats_;
  uint32_t last_acked_frame_;
  uint32_t last_encoded_frame_;
  base::TimeDelta rtt_;
...
class FixedCongestionControl : public CongestionControl {
 public:
  FixedCongestionControl(uint32_t bitrate) : bitrate_(bitrate) {}
...
CongestionControl* NewFixedCongestionControl(uint32_t bitrate) {
...
AdaptiveCongestionControl::AdaptiveCongestionControl(
    base::TickClock* clock, uint32_t max_bitrate_configured,
    uint32_t min_bitrate_configured, double max_frame_rate)
    : clock_(clock),
      ...
      last_encoded_frame_(static_cast<uint32_t>(-1)),
      history_size_(kHistorySize),
      acked_bits_in_history_(0) {
  PP_DCHECK(max_bitrate_configured >= min_bitrate_configured);
  ...
  PP_DCHECK(!frame_stats_[0].ack_time.is_null());
}
...
void AdaptiveCongestionControl::UpdateTargetPlayoutDelay(
  ...
  const int max_unacked_frames = std::min(
      kMaxUnackedFrames, 1 + static_cast<int>(delay * max_frame_rate_ /
                                              base::TimeDelta::FromSeconds(1)));
  PP_DCHECK(max_unacked_frames > 0);
...
AdaptiveCongestionControl::FrameStats* AdaptiveCongestionControl::GetFrameStats(
  ...
  PP_DCHECK(offset < static_cast<int32_t>(kHistorySize));
  if (offset > 0) {
  ...
  if (offset < 0 || offset >= static_cast<int32_t>(frame_stats_.size())) {
    return NULL;
  }
...
void AdaptiveCongestionControl::PruneFrameStats() {
  while (frame_stats_.size() > history_size_) {
    PP_DCHECK(frame_stats_.size() > 1UL);
    PP_DCHECK(!frame_stats_[0].ack_time.is_null());
    acked_bits_in_history_ -= frame_stats_[0].frame_size;
    dead_time_in_history_ -= DeadTime(frame_stats_[0], frame_stats_[1]);
    PP_DCHECK(acked_bits_in_history_ >= 0UL);
    DINF() << "DT: " << dead_time_in_history_.InSecondsF();
    PP_DCHECK(dead_time_in_history_.InSecondsF() >= 0.0);
...
void AdaptiveCongestionControl::AckFrame(uint32_t frame_id,
  ...
  while (IsNewerFrameId(frame_id, last_acked_frame_)) {
    ...
    PP_DCHECK(frame_stats);
    if (frame_stats->sent_time.is_null()) {
      // Can't ack a frame that hasn't been sent yet.
      return;
    }
    last_acked_frame_++;
    if (when < frame_stats->sent_time) when = frame_stats->sent_time;
...
base::TimeTicks AdaptiveCongestionControl::EstimatedAckTime(uint32_t frame_id,
                                                            double bitrate) {
  FrameStats* frame_stats = GetFrameStats(frame_id);
  PP_DCHECK(frame_stats);
  if (frame_stats->ack_time.is_null()) {
    PP_DCHECK(frame_stats->frame_size);
...
base::TimeTicks AdaptiveCongestionControl::EstimatedSendingTime(
    uint32_t frame_id, double bitrate) {
  FrameStats* frame_stats = GetFrameStats(frame_id);
  PP_DCHECK(frame_stats);
...
uint32_t AdaptiveCongestionControl::GetBitrate(base::TimeTicks playout_time,
  ...
  uint32_t bits_per_second = static_cast<uint32_t>(
      safe_bitrate * empty_buffer_fraction / kTargetEmptyBufferFraction);
  DINF() << " FBR:" << (bits_per_second / 1E6)
         << " EBF:" << empty_buffer_fraction << " SBR:" << (safe_bitrate / 1E6);
